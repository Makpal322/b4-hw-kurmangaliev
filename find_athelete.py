...
def nearest_by_birthday(user, session):
    athletes_list = session.query(Athelete).all()
    athlete_id_bd = {}
    for athlete in athletes_list:
        bd = str_to_date(athlete.birthdate)
...
def main():
    ...
    if not user:
        ...
        bd_athlete, bd = nearest_by_birthday(user, session)
        height_athlete, height = nearest_by_height(user, session)
        print(
            "Ближайший по дате рождения: {}, его дата рождения: {}".format(bd_athlete, bd)
        )
        print(
            "Ближайший по росту: {}, его рост: {}".format(height_athlete, height)
        )
